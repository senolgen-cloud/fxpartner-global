...
import { db } from "@/db";
import { tradeSignals, type TradeSignalOutcome } from "@/db/schema";
...
// Keeps the page reasonably fresh without hitting the DB on literally every
// request — signals don't open/close often enough to need shorter than this.
    ...
    <div className="rounded-2xl border border-hairline bg-ink-soft p-5">
      ...
      {resultLine && (
        <p
          ...
          style={{ color: outcomeColor(signal.outcome) }}
        >
          {resultLine}
        </p>
      )}
        ...
        <section className="border-b border-hairline">
          <div className="mx-auto max-w-6xl px-6 py-16">
            ...
            <p className="mt-4 max-w-2xl text-text-on-ink-muted">
              Every signal below comes straight from our tracked MT5 account
              through an automated EA — the same entries posted to our
              Telegram channel and X, with a real, verified result once each
              trade closes. Nothing here is simulated or backfilled.
            </p>
        ...
        <section className="border-t border-hairline">
          <div className="mx-auto max-w-3xl px-6 py-14">
            <p className="font-mono text-xs leading-relaxed text-text-on-ink-muted">
              These are real trades taken on FXPARTNER&apos;s own tracked MT5
              account, shared for informational purposes only — not
              investment advice. Past results don&apos;t guarantee future
              ones; always size positions and set stops according to your own
              risk tolerance.
            </p>
          </div>
        </section>
